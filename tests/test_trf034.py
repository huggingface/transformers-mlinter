...
"""
    ...
    def test_trf034_does_not_report_a_base_class_a_grandfathered_model_owns(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # A decoder layer, not one of the conv/pooling names the rule exempts outright, so what is
            # under test really is where the cutoff is read.
...
class DFineDecoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.layers = nn.ModuleList([DFineDecoderLayer(config) for _ in range(2)])
"""
