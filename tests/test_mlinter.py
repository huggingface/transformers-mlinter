...
from mlinter import trf052 as _trf052_mod
...
"""
    ...
    # --- TRF022: config fields must use canonical dimension names ---

    def _trf022(self, source, file_name="configuration_foo.py"):
        file_path = Path(f"src/transformers/models/foo/{file_name}")
    ...
    def test_trf022_flags_legacy_dataclass_fields(self):
        source = """
...
class FooConfig(PreTrainedConfig):
    ...
    n_layers: int = 24
"""
        violations = self._trf022(source)
    ...
    def test_trf022_flags_legacy_init_assignment(self):
        source = """
class FooConfig(PreTrainedConfig):
...
"""
    ...
    def test_trf022_accepts_canonical_names(self):
        source = """
@strict(accept_kwargs=True)
class FooConfig(PreTrainedConfig):
    ...
    embed_dim: int = 512
"""
        self.assertEqual(self._trf022(source), [])

    def test_trf022_reports_each_legacy_field_once(self):
        source = """
class FooConfig(PreTrainedConfig):
...
"""
        self.assertEqual(len(self._trf022(source)), 1)

    def test_trf022_ignores_non_config_classes_and_files(self):
        source = """
class FooAttention(nn.Module):
    d_model: int = 1024
"""
        ...
        self.assertEqual(self._trf022(config_source, file_name="modeling_foo.py"), [])

    def test_trf022_respects_suppression(self):
        source = """
class FooConfig(PreTrainedConfig):
    # trf-ignore: TRF022
    d_model: int = 1024
"""
    ...
    def test_trf022_exempts_models_before_cutoff(self):
        source = "class FooConfig(PreTrainedConfig):\n    d_model: int = 1024\n"
        file_path = Path("src/transformers/models/foo/configuration_foo.py")
        with patch.object(_helpers_mod, "model_contribution_date", return_value=date(2023, 1, 1)):
            with patch.object(_trf022_mod, "CUTOFF_DATE", "2026-06-20"):
                violations = mlinter.analyze_file(file_path, source, enabled_rules={mlinter.TRF022})
        self.assertEqual([v for v in violations if v.rule_id == mlinter.TRF022], [])

    # --- TRF023: layer dimensions must come from the config ---

    def _trf023(self, source, file_name="modeling_foo.py"):
        file_path = Path(f"src/transformers/models/foo/{file_name}")
        with patch.object(_helpers_mod, "model_contribution_date", return_value=None):
    ...
    def test_trf023_flags_hardcoded_dimensions(self):
        source = """
...
"""
    ...
    def test_trf023_flags_keyword_dimensions_and_sequence_shapes(self):
        source = """
class FooEmbedding(nn.Module):
    def __init__(self, config):
        super().__init__()
...
"""
    ...
    def test_trf023_allows_config_values_and_small_literals(self):
        source = """
...
"""
    ...
    def test_trf023_ignores_operator_shape_arguments(self):
        source = """
...
"""
    ...
    def test_trf023_ignores_unrelated_linear_attribute(self):
        source = """
...
"""
    ...
    def test_trf023_respects_suppression_and_file_type(self):
        source = """
class FooEmbedding(nn.Module):
    def __init__(self, config):
        super().__init__()
...
"""
        ...
        self.assertEqual(self._trf023(plain, file_name="configuration_foo.py"), [])

    # --- TRF024: masks must be built once in the model, not per layer ---
    ...
    def test_trf024_flags_mask_creation_inside_a_layer(self):
        source = """
...
"""
    ...
    def test_trf024_flags_custom_mask_factory_in_attention(self):
        source = """
class FooAttention(nn.Module):
...
"""
        self.assertEqual(len(self._trf024(source)), 1)

    def test_trf024_allows_mask_creation_in_the_model(self):
        source = """
class FooModel(FooPreTrainedModel):
...
"""
    ...
    def test_trf024_allows_layer_consuming_a_prepared_mask(self):
        source = """
class FooDecoderLayer(nn.Module):
    def forward(self, hidden_states, attention_mask=None, **kwargs):
        return self.self_attn(hidden_states, attention_mask, **kwargs)
...
"""
        self.assertEqual(self._trf024(plain, file_name="processing_foo.py"), [])

    # --- TRF025: a module that only forwards to its single submodule ---
    ...
    def test_trf025_flags_pass_through_wrapper(self):
        source = """
...
"""
    ...
    def test_trf025_flags_wrapper_with_docstring(self):
        source = '''
...
'''
    ...
    def test_trf025_allows_module_doing_extra_work(self):
        source = """
class FooBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
...
"""
    ...
    def test_trf025_allows_extra_statement_or_method(self):
        residual = """
class FooBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
...
"""
        self.assertEqual(self._trf025(residual), [])
        extra_method = """
class FooBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, hidden_states, **kwargs):
        return self.encoder(hidden_states, **kwargs)
"""
        self.assertEqual(self._trf025(extra_method), [])

    def test_trf025_exempts_pretrained_model_subclasses(self):
        source = """
class FooModel(FooPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
"""
    ...
    def test_trf025_allows_delegating_to_a_different_attribute(self):
        source = """
class FooBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
...
"""
    ...
    def test_trf025_respects_suppression(self):
        source = """
# trf-ignore: TRF025
class FooAtomTransformer(nn.Module):
    def __init__(self, config):
...
"""
    ...
    # --- TRF051: no .item()/.tolist() inside forward ---

    def test_trf051_flags_tolist_in_forward(self):
        source = """
...
"""
        violations = self._run(mlinter.TRF051, source)
        self.assertEqual(len(violations), 1)
        self.assertIn(".tolist()", violations[0].message)

    def test_trf051_flags_item_in_forward(self):
        source = """
class FooModel(FooPreTrainedModel):
...
"""
    ...
    def test_trf051_exempts_split_sizes(self):
        source = """
class FooAttention(nn.Module):
...
"""
        self.assertEqual(self._run(mlinter.TRF051, source), [])

    def test_trf051_exempts_split_sizes_through_a_local(self):
        source = """
class FooVisionModel(FooPreTrainedModel):
    def forward(self, hidden_states, grid_thw):
...
"""
    ...
    def test_trf051_ignores_calls_outside_forward(self):
        source = """
class FooModel(FooPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
"""
    ...
    def test_trf051_respects_suppression(self):
        source = """
class FooModel(FooPreTrainedModel):
...
"""
    ...
    # --- TRF052: @auto_docstring on the documented classes and their entry points ---

    def _trf052_modular(self, modular_source, generated_files):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(_helpers_mod, "model_contribution_date", return_value=None):
                violations = mlinter.analyze_file(file_path, modular_source, enabled_rules={mlinter.TRF052})
            return [v for v in violations if v.rule_id == mlinter.TRF052]

    def test_trf052_flags_undecorated_model_class(self):
        source = """
class FooModel(FooPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
"""
        violations = self._run(mlinter.TRF052, source)
        self.assertEqual(len(violations), 1)
        self.assertIn("`FooModel` is a public class", violations[0].message)

    def test_trf052_accepts_decorated_model_class(self):
        source = """
@auto_docstring
class FooForConditionalGeneration(FooPreTrainedModel, GenerationMixin):
    def __init__(self, config):
        super().__init__(config)
"""
        self.assertEqual(self._run(mlinter.TRF052, source), [])

    def test_trf052_ignores_inner_model_class_names(self):
        source = """
...
"""
    ...
    def test_trf052_ignores_generic_task_mixin_subclasses(self):
        source = """
...
"""
    ...
    def test_trf052_flags_undecorated_output_class(self):
        source = """
@dataclass
...
"""
    ...
    def test_trf052_asks_for_the_innermost_position_without_an_inner_decorator(self):
        source = """
@register_config
class FooConfig(PreTrainedConfig):
    hidden_size: int = 768
"""
        violations = self._run(mlinter.TRF052, source, file_name="configuration_foo.py")
        self.assertEqual(len(violations), 1)
        self.assertIn("Add @auto_docstring as the innermost decorator", violations[0].message)

    def test_trf052_asks_for_the_strict_position(self):
        source = """
@strict(accept_kwargs=True)
class FooConfig(PreTrainedConfig):
    hidden_size: int = 768
"""
    ...
    def test_trf052_prefers_the_strict_position_over_the_dataclass_one(self):
        source = """
@strict
@dataclass
class FooModelOutputWithPast(ModelOutput):
...
"""
    ...
    def test_trf052_asks_for_the_dataclass_position_in_a_modular_file(self):
        modular_source = """
...
class FooModelOutputWithPast(ModelOutput):
    logits: torch.FloatTensor | None = None
"""
        violations = self._trf052_modular(modular_source, {"modeling_foo.py": generated})
        self.assertEqual(len(violations), 1)
        self.assertIn("Add @auto_docstring above @dataclass", violations[0].message)

    def test_trf052_flags_undecorated_config_class(self):
        source = """
@strict(accept_kwargs=True)
class FooConfig(PreTrainedConfig):
    hidden_size: int = 768
"""
    ...
    def test_trf052_accepts_decorated_config_class(self):
        source = """
@strict(accept_kwargs=True)
...
class FooTextConfig(FooConfig):
    hidden_size: int = 768
"""
        self.assertEqual(self._run(mlinter.TRF052, source, file_name="configuration_foo.py"), [])

    def test_trf052_flags_undecorated_processor_class(self):
        source = """
class FooProcessorKwargs(ProcessingKwargs, total=False):
    _defaults = {}
...
class FooProcessor(ProcessorMixin):
    ...
    def __call__(self, images=None, text=None, **kwargs):
        return BatchFeature()
"""
        violations = self._run(mlinter.TRF052, source, file_name="processing_foo.py")
        self.assertEqual(len(violations), 1)
        self.assertIn("`FooProcessor` is a public class", violations[0].message)

    def test_trf052_ignores_classes_without_bases(self):
        source = """
class AutoProcessor:
    def __call__(self, *args, **kwargs):
        raise EnvironmentError("Use from_pretrained")
"""
        self.assertEqual(self._run(mlinter.TRF052, source, file_name="processing_foo.py"), [])

    def test_trf052_flags_undecorated_forward(self):
        source = """
@auto_docstring
class FooModel(FooPreTrainedModel):
    def forward(self, input_ids):
...
"""
    ...
    def test_trf052_accepts_decorated_forward(self):
        source = """
@auto_docstring
class FooModel(FooPreTrainedModel):
...
"""
    ...
    def test_trf052_flags_undecorated_feature_getters(self):
        source = """
@auto_docstring
class FooForConditionalGeneration(FooPreTrainedModel):
...
"""
    ...
    def test_trf052_ignores_inner_layer_forward(self):
        source = """
class FooAttention(nn.Module):
    def forward(self, hidden_states):
        return hidden_states
...
"""
    ...
    def test_trf052_flags_undecorated_preprocess(self):
        source = """
@auto_docstring
class FooImageProcessor(TorchvisionBackend):
    def preprocess(self, images, **kwargs):
        return images
"""
        violations = self._run(mlinter.TRF052, source, file_name="image_processing_foo.py")
        self.assertEqual(len(violations), 1)
        self.assertIn("`FooImageProcessor.preprocess` is a public method", violations[0].message)

    def test_trf052_accepts_decorated_preprocess(self):
        source = """
@auto_docstring
class FooImageProcessor(PilBackend):
    ...
    def preprocess(self, images, **kwargs):
        return images
"""
        self.assertEqual(self._run(mlinter.TRF052, source, file_name="image_processing_pil_foo.py"), [])

    def test_trf052_flags_undecorated_processor_call(self):
        source = """
@auto_docstring
class FooProcessor(ProcessorMixin):
    def __call__(self, images=None, text=None, **kwargs):
...
"""
    ...
    def test_trf052_flags_processor_whose_base_is_another_model(self):
        source = """
...
"""
    ...
    def test_trf052_ignores_helper_call_in_an_image_processing_file(self):
        source = """
class FooMaskingGenerator:
    def __call__(self):
        return self.mask
"""
        self.assertEqual(self._run(mlinter.TRF052, source, file_name="image_processing_foo.py"), [])

    def test_trf052_ignores_video_processors(self):
        source = """
class FooVideoProcessor(BaseVideoProcessor):
    def preprocess(self, videos, **kwargs):
        return videos
"""
        self.assertEqual(self._run(mlinter.TRF052, source, file_name="video_processing_foo.py"), [])

    def test_trf052_respects_suppression(self):
        source = """
# trf-ignore: TRF052
class FooModel(FooPreTrainedModel):
    # trf-ignore: TRF052
...
"""
    ...
    def test_trf052_exempts_models_before_cutoff(self):
        source = """
class FooModel(FooPreTrainedModel):
    def forward(self, input_ids):
        return self.layers(input_ids)
"""
        file_path = Path("src/transformers/models/foo/modeling_foo.py")
        with patch.object(_helpers_mod, "model_contribution_date", return_value=date(2023, 1, 1)):
            with patch.object(_trf052_mod, "CUTOFF_DATE", "2026-06-20"):
    ...
    def test_trf052_modular_class_and_method_inherit_the_decorator(self):
        modular_source = """
...
class FooModel(FooPreTrainedModel):
    ...
    def forward(self, input_ids):
        return self.layers(input_ids)
"""
        self.assertEqual(self._trf052_modular(modular_source, {"modeling_foo.py": generated}), [])

    def test_trf052_flags_modular_class_that_ships_undecorated(self):
        modular_source = """
from ..llama.modeling_llama import LlamaModel
...
"""
    ...
    def test_trf052_flags_modular_override_that_ships_undecorated(self):
        modular_source = """
from ..llama.modeling_llama import LlamaModel
...
"""
    ...
    def test_trf052_reports_dropped_parent_decorators(self):
        modular_source = """
from ..llama.modeling_llama import LlamaModel
...
"""
    ...
    def test_trf052_accepts_modular_decorator_before_regeneration(self):
        modular_source = """
from ..llama.modeling_llama import LlamaModel
...
class FooModel(FooPreTrainedModel):
    def forward(self, input_ids):
        return self.layers(input_ids)
"""
        self.assertEqual(self._trf052_modular(modular_source, {"modeling_foo.py": stale_generated}), [])

    def test_trf052_ignores_modular_method_deleted_by_the_converter(self):
        modular_source = """
from ..llama.modeling_llama import LlamaModel
...
"""
    ...
    def test_trf052_finds_the_generated_file_by_banner_not_by_name(self):
        modular_source = """
...
class FooImageProcessorPil(PilBackend):
    def preprocess(self, images, **kwargs):
        return images
"""
        violations = self._trf052_modular(modular_source, {"image_processing_pil_foo.py": generated})
        self.assertEqual(len(violations), 1)
        self.assertIn("in image_processing_pil_foo.py", violations[0].message)

    def test_trf052_ignores_modular_class_absent_from_the_generated_files(self):
        modular_source = """
from ..llama.modeling_llama import LlamaModel
...
class FooModel(LlamaModel):
    def forward(self, input_ids):
        return super().forward(input_ids)
"""
        self.assertEqual(self._trf052_modular(modular_source, {}), [])
